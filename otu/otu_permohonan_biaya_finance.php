<?php include ('connect.php');
    ...
    $cek_apakah_saya_top_leader = mysql_fetch_array(mysql_query("SELECT * FROM tb_department WHERE top_leader = '$_SESSION[ID_karyawan]'"));
        ...
        }else if($PB['status_pb_finance']=='FINALIZE' && $PB['finance_check']=='CHECKED'){
        ...
        if(isset($_GET['tolak'])){
            ...
            $approve = mysql_query("UPDATE `tb_permohonan_biaya_master` SET status_pb='REVISI' , leader_check='' , status_pb_finance ='' WHERE tb_permohonan_biaya_master.ID_master_pb = '$_GET[tolak]'");
      ...
      if($terima_pb){
             // $kirim_feedback_ke_karyawan = send_wa($data_kar['no_telp'],"Permohonan Biaya Anda Pada Tanggal *".$date_time."* Sudah Diterima Oleh *".$data_finan['nama_lengkap']."* Dengan Status *".$data_kar['status_pb_finance']."* Klik link berikut ini :",'PERMOHONAN BIAYA','inboard.ardgroup.co.id');

             // $kirim_feedback_ke_leader = send_wa($nomortlp_top_leader,"Permohonan Biaya *".$data_kar['nama_lengkap']."* Pada Tanggal *".$date_time."* Anda Sudah Diterima Oleh *".$data_finan['nama_lengkap']."* Dengan Status *".$data_kar['status_pb_finance']."* Klik link berikut ini :",'PERMOHONAN BIAYA','inboard.ardgroup.co.id');

             $kirim_feedback_ke_finance = send_wa('6281291305529',"Permohonan Biaya *".$data_kar['nama_lengkap']."* Pada Tanggal *".$date_time."* Sudah Diterima Oleh *".$data_finan['nama_lengkap']."* Sekarangan giliran anda, klik link dibawah ini : ",'PERMOHONAN BIAYA','https://xeniel.5dapps.com/inboard/otu/otu_permohonan_biaya_finance.php?ID='.$id_PB.'');
  ...
  <div class="row">
    ...
    <div class="col-5">
    	...
    	if($data['status_pb_finance']=='MENUNGGU FINANCE'){
    	 ?>
      <a href="https://xeniel.5dapps.com/inboard/otu/otu_permohonan_biaya_finance.php?ID=<?php echo $data['ID_master_pb'] ?>&terima_finance=<?php echo $data[0] ?>&telpon=<?php echo $data['no_telp'] ?>&telepon_atasan=<?php echo $telepon_atasan['no_telp'] ?>&top_leader=<?php echo $data['approved_by'] ?>"><span class="btn btn-success btn-growl" data-growl="success"><i class="fa fa-check-circle"></i>TERIMA PB</span></a>
      <a href="https://xeniel.5dapps.com/inboard/otu/otu_permohonan_biaya_finance.php?ID=<?php echo $data['ID_master_pb'] ?>&tolak=<?php echo $data[0] ?>&ditolak=<?php echo $data['no_telp'] ?>&lead=<?php echo $data['approved_by'] ?>"><span class="btn btn-danger btn-growl" data-growl="success"><i class="fa fa-times-circle-o"></i> NGOBROL YUK</span></a>
  <?php }else if($data['status_pb_finance']=='PB DITERIMA'){
  ?>
    <a href="https://xeniel.5dapps.com/inboard/otu/otu_permohonan_biaya_finance.php?ID=<?php echo $data['ID_master_pb'] ?>&proses_pb=<?php echo $data[0] ?>&telpon=<?php echo $data['no_telp'] ?>&telepon_atasan=<?php echo $telepon_atasan['no_telp'] ?>&top_leader=<?php echo $data['approved_by'] ?>"><span class="btn btn-success btn-growl" data-growl="success"><i class="fa fa-check-circle"></i>PROSES PB</span></a>
    <a href="https://xeniel.5dapps.com/inboard/otu/otu_permohonan_biaya_finance.php?ID=<?php echo $data['ID_master_pb'] ?>&tolak=<?php echo $data[0] ?>&ditolak=<?php echo $data['no_telp'] ?>&lead=<?php echo $data['approved_by'] ?>"><span class="btn btn-danger btn-growl" data-growl="success"><i class="fa fa-times-circle-o"></i> NGOBROL YUK</span></a>
   <?php }else{?>
   	<a href="https://xeniel.5dapps.com/inboard/otu/otu_permohonan_biaya_finance.php?ID=<?php echo $data[0] ?>&id_karyawan=<?php echo $data['id_pemohon'] ?>&id_lead=<?php echo $data['approved_by'] ?>" class="btn btn-effect-ripple btn-danger" data-toggle="modal" style="float:right; ">UPLOAD BUKTI TRANSFER</a>
   <?php } ?>
  </div>
  <div class="col-1"></div>
  <div class="col-5">
