...
<body>
  ...
  <div class="container">
    ...
    <div class="row">
      <div class="col-1"></div>
      <div class="col-11">
        Berikut permohonan pembatalan cuti dengan informasi sebagai berikut:
        <br><br>
        <?php 
        if(isset($_GET['delete_pembatalan'])){
          ...
          if($delete_batal){
            ...
            $kirim_feedback_ke_karyawan = send_wa($telp_karyawan,'PEMBATALAN CUTI ANDA TELAH DI SETUJUI, klik link dibawah ini : ','CUTI','http://inboard.ardgroup.co.id');
            echo "<script>document.location.href='https://xeniel.5dapps.com/inboard/otu/otu_respond.php'</script>";
          }
        }
      ...
      <div class="row">
        <div class="col-1"></div>
        <div class="col-5">
          <a onclick="return confirm('Apakah anda yakin? Aksi ini tidak dapat di ulangi')" href="https://xeniel.5dapps.com/inboard/otu/otu_permohonan.php?ID=<?php echo $data['ID_permohonan_cuti'] ?>&delete_pembatalan=<?php echo $data['ID_permohonan_cuti'] ?>&telp=<?php echo $data['no_telp'] ?>" data-toggle="tooltip" title="Terima" class="btn btn-effect-ripple btn-xs btn-success"><i class="fa fa-check">TERIMA PERMOHONAN</i></a>

        </div>
        <div class="col-1"></div>
        <div class="col-5">
          <a onclick="return confirm('Apakah anda yakin? Aksi ini tidak dapat di ulangi')" href="https://xeniel.5dapps.com/inboard/otu/otu_permohonan_cuti.php?tolak_pembatalan=<?php echo $data['ID_permohonan_cuti'] ?>&telpon=<?php echo $data['no_telp'] ?>" data-toggle="tooltip" title="Delete" class="btn btn-effect-ripple btn-xs btn-danger"><i class="fa fa-close">NGOBROL YUK</i></a>
        </div>
      </div>
    <?php } ?>
